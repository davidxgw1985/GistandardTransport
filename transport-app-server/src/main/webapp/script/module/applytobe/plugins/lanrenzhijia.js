...
function PathRun(){
	...
	if(PathStatus == 0){
		...
		PathMenu.addClass('active').removeClass('noActive');
		PathStatus = 1;
		$('.action-btn').animate({marginRight:"3.8rem"});
		...
		$('.action-btn').animate({marginRight:"1.866667rem"});
		PathMenu.addClass('noActive').removeClass('active');
		if(mainButton[1]['angle']){
