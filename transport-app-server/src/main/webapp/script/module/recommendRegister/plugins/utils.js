...
var checkVal = function(val,type){
	...
	switch (type){
		...
		// 大小写密码
		case 6:
			if(/^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])[a-zA-Z0-9]{8,20}$/.test(val)) {
				return true;
			}
			return false;
			break;
...
var showDialogEndNum = function ($container,html,cfmcallback,calcallback) {
	var t = simpleTpl(),
		$container = $container || $('body'),
		$loading = $container ? $container.find('#gis_loading') : $('body').children('#gis_loading');
	...
	t._('<div class="showDialog">')
		._('<div class="st-content" style="margin-top:'+(($(window).height()-600)/2)+'px">')
		._('<p class="st-title cf">请选择账号末尾八位数</p>')
		._('<div class="st-end-num">')
		._('<ul>')
		._(html)
		._('</ul>')
		._('</div>')
		...
		._('</div>')
		._('<div class=" btn">')
		._('<div class="btn-cancle">关&nbsp;闭</div>')
		._('</div>')
		._('</div>')
		._('</div>')
		._('</div>')
