package com.gistandard.transport.webdubbo.auditAccount.bean;

import java.io.Serializable;
import java.math.BigDecimal;
import java.util.Date;
import java.util.List;

/**
 * 企业信息
 */
public class CustomerInfo implements Serializable{

    // 员工人数
    private String staffNumStr;

    // 服务范围
    private String serviceScopeStr;

    // 自有车辆
    private String selfOwnerCarNumStr;

    // 往来币种
    private String currCodeStr;

    // 注册币种
    private String registerFundCurrCodeStr;

    // 成立时间
    private String foundedYearsStr;

    // 站点类型：只有站点有该属性
    private List<String> stationTypeList;


    private Integer id;

    private String customNo;

    private String custTtl;

    private String custClas;

    private String custRanc;

    private String custName;

    private String custEng;

    private String custRoot;

    private String custArea;

    private String custGild;

    private String comskind;

    private String custMan;

    private String custMantel;

    private String custAdd;

    private String detailAdd;

    private String custPost;

    private String identno;

    private String country;

    private String province;

    private String city;

    private String county;

    private String custEAdd;

    private String custEPost;

    private String custTel;

    private String custFax;

    private String custMail;

    private String custWeb;

    private String flinkMan;

    private String fplace;

    private String fworkTel;

    private String fworkFax;

    private String fmobile;

    private String femail;

    private String fhometel;

    private String slinkman;

    private String splace;

    private String sworktel;

    private String sworkfax;

    private String smobile;

    private String semail;

    private String shometel;

    private String custComno;

    private String custTaxno;

    private String currCode;

    private String custBank;

    private String custBankno;

    private String receikind;

    private BigDecimal promptdays;

    private String settlekind;

    private BigDecimal settledays;

    private String creditranc;

    private BigDecimal creditmax;

    private String custvmmsno;

    private String custintro;

    private String narrate;

    private String deptoun;

    private Byte ispublic;

    private String salesno;

    private Date notedate;

    private Integer accountId;

    private BigDecimal staLongitude;

    private BigDecimal staLatitude;

    private String prefixCode;

    private String customsCo;

    private String serviceScope;

    private Integer registerFund;

    private String registerFundCurrCode;

    private Date foundedYears;

    private String staffNum;

    private BigDecimal areaNum;

    private Integer mainLineCarNum;

    private Integer selfOwnerCarNum;

    private String selfOwnerCarNumDesc;

    private String roadTranPermitNo;

    private String orgCodeLilcenseNo;

    private Integer serviceType;

    public String getStaffNumStr() {
        return staffNumStr;
    }

    public void setStaffNumStr(String staffNumStr) {
        this.staffNumStr = staffNumStr;
    }

    public String getServiceScopeStr() {
        return serviceScopeStr;
    }

    public void setServiceScopeStr(String serviceScopeStr) {
        this.serviceScopeStr = serviceScopeStr;
    }

    public String getSelfOwnerCarNumStr() {
        return selfOwnerCarNumStr;
    }

    public void setSelfOwnerCarNumStr(String selfOwnerCarNumStr) {
        this.selfOwnerCarNumStr = selfOwnerCarNumStr;
    }

    public String getCurrCodeStr() {
        return currCodeStr;
    }

    public void setCurrCodeStr(String currCodeStr) {
        this.currCodeStr = currCodeStr;
    }

    public String getRegisterFundCurrCodeStr() {
        return registerFundCurrCodeStr;
    }

    public void setRegisterFundCurrCodeStr(String registerFundCurrCodeStr) {
        this.registerFundCurrCodeStr = registerFundCurrCodeStr;
    }

    public String getFoundedYearsStr() {
        return foundedYearsStr;
    }

    public void setFoundedYearsStr(String foundedYearsStr) {
        this.foundedYearsStr = foundedYearsStr;
    }

    public List<String> getStationTypeList() {
        return stationTypeList;
    }

    public void setStationTypeList(List<String> stationTypeList) {
        this.stationTypeList = stationTypeList;
    }

    public Integer getId() {
        return id;
    }

    public void setId(Integer id) {
        this.id = id;
    }

    public String getCustomNo() {
        return customNo;
    }

    public void setCustomNo(String customNo) {
        this.customNo = customNo;
    }

    public String getCustTtl() {
        return custTtl;
    }

    public void setCustTtl(String custTtl) {
        this.custTtl = custTtl;
    }

    public String getCustClas() {
        return custClas;
    }

    public void setCustClas(String custClas) {
        this.custClas = custClas;
    }

    public String getCustRanc() {
        return custRanc;
    }

    public void setCustRanc(String custRanc) {
        this.custRanc = custRanc;
    }

    public String getCustName() {
        return custName;
    }

    public void setCustName(String custName) {
        this.custName = custName;
    }

    public String getCustEng() {
        return custEng;
    }

    public void setCustEng(String custEng) {
        this.custEng = custEng;
    }

    public String getCustRoot() {
        return custRoot;
    }

    public void setCustRoot(String custRoot) {
        this.custRoot = custRoot;
    }

    public String getCustArea() {
        return custArea;
    }

    public void setCustArea(String custArea) {
        this.custArea = custArea;
    }

    public String getCustGild() {
        return custGild;
    }

    public void setCustGild(String custGild) {
        this.custGild = custGild;
    }

    public String getComskind() {
        return comskind;
    }

    public void setComskind(String comskind) {
        this.comskind = comskind;
    }

    public String getCustMan() {
        return custMan;
    }

    public void setCustMan(String custMan) {
        this.custMan = custMan;
    }

    public String getCustMantel() {
        return custMantel;
    }

    public void setCustMantel(String custMantel) {
        this.custMantel = custMantel;
    }

    public String getCustAdd() {
        return custAdd;
    }

    public void setCustAdd(String custAdd) {
        this.custAdd = custAdd;
    }

    public String getDetailAdd() {
        return detailAdd;
    }

    public void setDetailAdd(String detailAdd) {
        this.detailAdd = detailAdd;
    }

    public String getCustPost() {
        return custPost;
    }

    public void setCustPost(String custPost) {
        this.custPost = custPost;
    }

    public String getIdentno() {
        return identno;
    }

    public void setIdentno(String identno) {
        this.identno = identno;
    }

    public String getCountry() {
        return country;
    }

    public void setCountry(String country) {
        this.country = country;
    }

    public String getProvince() {
        return province;
    }

    public void setProvince(String province) {
        this.province = province;
    }

    public String getCity() {
        return city;
    }

    public void setCity(String city) {
        this.city = city;
    }

    public String getCounty() {
        return county;
    }

    public void setCounty(String county) {
        this.county = county;
    }

    public String getCustEAdd() {
        return custEAdd;
    }

    public void setCustEAdd(String custEAdd) {
        this.custEAdd = custEAdd;
    }

    public String getCustEPost() {
        return custEPost;
    }

    public void setCustEPost(String custEPost) {
        this.custEPost = custEPost;
    }

    public String getCustTel() {
        return custTel;
    }

    public void setCustTel(String custTel) {
        this.custTel = custTel;
    }

    public String getCustFax() {
        return custFax;
    }

    public void setCustFax(String custFax) {
        this.custFax = custFax;
    }

    public String getCustMail() {
        return custMail;
    }

    public void setCustMail(String custMail) {
        this.custMail = custMail;
    }

    public String getCustWeb() {
        return custWeb;
    }

    public void setCustWeb(String custWeb) {
        this.custWeb = custWeb;
    }

    public String getFlinkMan() {
        return flinkMan;
    }

    public void setFlinkMan(String flinkMan) {
        this.flinkMan = flinkMan;
    }

    public String getFplace() {
        return fplace;
    }

    public void setFplace(String fplace) {
        this.fplace = fplace;
    }

    public String getFworkTel() {
        return fworkTel;
    }

    public void setFworkTel(String fworkTel) {
        this.fworkTel = fworkTel;
    }

    public String getFworkFax() {
        return fworkFax;
    }

    public void setFworkFax(String fworkFax) {
        this.fworkFax = fworkFax;
    }

    public String getFmobile() {
        return fmobile;
    }

    public void setFmobile(String fmobile) {
        this.fmobile = fmobile;
    }

    public String getFemail() {
        return femail;
    }

    public void setFemail(String femail) {
        this.femail = femail;
    }

    public String getFhometel() {
        return fhometel;
    }

    public void setFhometel(String fhometel) {
        this.fhometel = fhometel;
    }

    public String getSlinkman() {
        return slinkman;
    }

    public void setSlinkman(String slinkman) {
        this.slinkman = slinkman;
    }

    public String getSplace() {
        return splace;
    }

    public void setSplace(String splace) {
        this.splace = splace;
    }

    public String getSworktel() {
        return sworktel;
    }

    public void setSworktel(String sworktel) {
        this.sworktel = sworktel;
    }

    public String getSworkfax() {
        return sworkfax;
    }

    public void setSworkfax(String sworkfax) {
        this.sworkfax = sworkfax;
    }

    public String getSmobile() {
        return smobile;
    }

    public void setSmobile(String smobile) {
        this.smobile = smobile;
    }

    public String getSemail() {
        return semail;
    }

    public void setSemail(String semail) {
        this.semail = semail;
    }

    public String getShometel() {
        return shometel;
    }

    public void setShometel(String shometel) {
        this.shometel = shometel;
    }

    public String getCustComno() {
        return custComno;
    }

    public void setCustComno(String custComno) {
        this.custComno = custComno;
    }

    public String getCustTaxno() {
        return custTaxno;
    }

    public void setCustTaxno(String custTaxno) {
        this.custTaxno = custTaxno;
    }

    public String getCurrCode() {
        return currCode;
    }

    public void setCurrCode(String currCode) {
        this.currCode = currCode;
    }

    public String getCustBank() {
        return custBank;
    }

    public void setCustBank(String custBank) {
        this.custBank = custBank;
    }

    public String getCustBankno() {
        return custBankno;
    }

    public void setCustBankno(String custBankno) {
        this.custBankno = custBankno;
    }

    public String getReceikind() {
        return receikind;
    }

    public void setReceikind(String receikind) {
        this.receikind = receikind;
    }

    public BigDecimal getPromptdays() {
        return promptdays;
    }

    public void setPromptdays(BigDecimal promptdays) {
        this.promptdays = promptdays;
    }

    public String getSettlekind() {
        return settlekind;
    }

    public void setSettlekind(String settlekind) {
        this.settlekind = settlekind;
    }

    public BigDecimal getSettledays() {
        return settledays;
    }

    public void setSettledays(BigDecimal settledays) {
        this.settledays = settledays;
    }

    public String getCreditranc() {
        return creditranc;
    }

    public void setCreditranc(String creditranc) {
        this.creditranc = creditranc;
    }

    public BigDecimal getCreditmax() {
        return creditmax;
    }

    public void setCreditmax(BigDecimal creditmax) {
        this.creditmax = creditmax;
    }

    public String getCustvmmsno() {
        return custvmmsno;
    }

    public void setCustvmmsno(String custvmmsno) {
        this.custvmmsno = custvmmsno;
    }

    public String getCustintro() {
        return custintro;
    }

    public void setCustintro(String custintro) {
        this.custintro = custintro;
    }

    public String getNarrate() {
        return narrate;
    }

    public void setNarrate(String narrate) {
        this.narrate = narrate;
    }

    public String getDeptoun() {
        return deptoun;
    }

    public void setDeptoun(String deptoun) {
        this.deptoun = deptoun;
    }

    public Byte getIspublic() {
        return ispublic;
    }

    public void setIspublic(Byte ispublic) {
        this.ispublic = ispublic;
    }

    public String getSalesno() {
        return salesno;
    }

    public void setSalesno(String salesno) {
        this.salesno = salesno;
    }

    public Date getNotedate() {
        return notedate;
    }

    public void setNotedate(Date notedate) {
        this.notedate = notedate;
    }

    public Integer getAccountId() {
        return accountId;
    }

    public void setAccountId(Integer accountId) {
        this.accountId = accountId;
    }

    public BigDecimal getStaLongitude() {
        return staLongitude;
    }

    public void setStaLongitude(BigDecimal staLongitude) {
        this.staLongitude = staLongitude;
    }

    public BigDecimal getStaLatitude() {
        return staLatitude;
    }

    public void setStaLatitude(BigDecimal staLatitude) {
        this.staLatitude = staLatitude;
    }

    public String getPrefixCode() {
        return prefixCode;
    }

    public void setPrefixCode(String prefixCode) {
        this.prefixCode = prefixCode;
    }

    public String getCustomsCo() {
        return customsCo;
    }

    public void setCustomsCo(String customsCo) {
        this.customsCo = customsCo;
    }

    public String getServiceScope() {
        return serviceScope;
    }

    public void setServiceScope(String serviceScope) {
        this.serviceScope = serviceScope;
    }

    public Integer getRegisterFund() {
        return registerFund;
    }

    public void setRegisterFund(Integer registerFund) {
        this.registerFund = registerFund;
    }

    public String getRegisterFundCurrCode() {
        return registerFundCurrCode;
    }

    public void setRegisterFundCurrCode(String registerFundCurrCode) {
        this.registerFundCurrCode = registerFundCurrCode;
    }

    public Date getFoundedYears() {
        return foundedYears;
    }

    public void setFoundedYears(Date foundedYears) {
        this.foundedYears = foundedYears;
    }

    public String getStaffNum() {
        return staffNum;
    }

    public void setStaffNum(String staffNum) {
        this.staffNum = staffNum;
    }

    public BigDecimal getAreaNum() {
        return areaNum;
    }

    public void setAreaNum(BigDecimal areaNum) {
        this.areaNum = areaNum;
    }

    public Integer getMainLineCarNum() {
        return mainLineCarNum;
    }

    public void setMainLineCarNum(Integer mainLineCarNum) {
        this.mainLineCarNum = mainLineCarNum;
    }

    public Integer getSelfOwnerCarNum() {
        return selfOwnerCarNum;
    }

    public void setSelfOwnerCarNum(Integer selfOwnerCarNum) {
        this.selfOwnerCarNum = selfOwnerCarNum;
    }

    public String getSelfOwnerCarNumDesc() {
        return selfOwnerCarNumDesc;
    }

    public void setSelfOwnerCarNumDesc(String selfOwnerCarNumDesc) {
        this.selfOwnerCarNumDesc = selfOwnerCarNumDesc;
    }

    public String getRoadTranPermitNo() {
        return roadTranPermitNo;
    }

    public void setRoadTranPermitNo(String roadTranPermitNo) {
        this.roadTranPermitNo = roadTranPermitNo;
    }

    public String getOrgCodeLilcenseNo() {
        return orgCodeLilcenseNo;
    }

    public void setOrgCodeLilcenseNo(String orgCodeLilcenseNo) {
        this.orgCodeLilcenseNo = orgCodeLilcenseNo;
    }

    public Integer getServiceType() {
        return serviceType;
    }

    public void setServiceType(Integer serviceType) {
        this.serviceType = serviceType;
    }
}
