...
import com.gistandard.transport.quote.module.services.strategy.ServiceQuoteStrategy;
...
import com.gistandard.transport.quote.system.common.define.AccountRoleType;
import com.gistandard.transport.quote.system.common.define.BelongType;
 ...
 */
public  abstract class AbstractServiceQuoteStrategy implements ServiceQuoteStrategy {
    ...
    public void setBaseQuoteBean(HttpServletRequest request) throws Exception{
        ...
        String kdOperateType = request.getParameter("kdOperateType");//快递报价操作类型
        ...
        //本地提派 开始结束路径 一致
       /* if(QuoteItem.BDTP.getId() ==  quoteBean.getComQuote().getItemId()){
            quoteBean.getComQuote().setEndRoute(quoteBean.getComQuote().getStartRoute());
        }*/
        if(quoteBean.getComQuote().getItemId() == 44){
            quoteBean.getComQuote().setKdOperateType(StringUtil.createInteger(kdOperateType));
        }

        //获取所属商户
        ComCustomer comCustomer = comCustomerDaoEx.queryCustomerInfoByAcctId(sysUser.getAccountId());
        //获取所属司机
        ComUserinfo comUserinfo = comUserinfoDaoEx.queryByAcctId(sysUser.getAccountId());

        if(AccountRoleType.OPERATOR_PERSONAL.getType()
                .equals(AccountRoleType.getTypeByOperatorRoleIds(sysUser.getComAccountRoleRels()))){
    ...
    public void setComCustomerDao(ComCustomerDao comCustomerDao) {
        this.comCustomerDao = comCustomerDao;
    }

    public void setComUserinfoDao(ComUserinfoDao comUserinfoDao) {
        this.comUserinfoDao = comUserinfoDao;
    }
}
