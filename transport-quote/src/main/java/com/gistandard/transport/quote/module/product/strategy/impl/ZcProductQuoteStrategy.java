 ...
 */
@Service("zcQuoteStrategy")
public class ZcProductQuoteStrategy extends AbstractProductQuoteStrategy {
    @Override
    public boolean checkQuoteParameter(QuoteBean quoteBean,ResultBean resultBean) {
        ...
        if (null == quoteBean.getComQuote().getTotalPrice()
                || (quoteBean.getComQuote().getTotalPrice().compareTo(NumberUtils.createBigDecimal("0"))<=0)) {
            resultBean.setState(false);
